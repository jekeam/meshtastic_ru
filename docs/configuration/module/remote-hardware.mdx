---
id: remote-hardware
title: Использование модуля Remote Hardware
sidebar_label: Remote Hardware
description: Модуль Remote Hardware позволяет читать, писать и отслеживать GPIO на удалённом Node.
---
...
Модуль Remote Hardware позволяет читать, писать и отслеживать GPIO на удалённом Node. Параметры: Enabled.
...
## Параметры Remote Hardware {#remote-hardware-config-values}

### Включено {#enabled}

Включен ли модуль.

## Доступность модуля Remote Hardware в клиентах {#remote-hardware-module-client-availability}
...
:::info заметка
Все параметры модуля Remote Hardware доступны в приложении для Android.

1. Откройте приложение Meshtastic
2. Перейдите: **Вертикальное многоточие (3 точки вверху справа) > Radio Configuration > Remote Hardware**
   :::
...
:::info заметка
Все параметры модуля Remote Hardware доступны в приложении для iOS, iPadOS и macOS: Settings > Module Configuration > Remote Hardware
:::
...
Все параметры модуля Remote Hardware доступны в Python CLI.
...
## Использование модуля Remote Hardware {#remote-hardware-module-usage}
...
- Получать уведомление по Mesh при изменении состояния любого GPIO.
  Учтите, быстрые изменения (например, короткие нажатия кнопки) не обнаруживаются. Для этого используйте модуль [Detection Sensor](/docs/configuration/module/detection-sensor).
...
2. Включите модуль Remote Hardware:
   ```shell
...
6. Включите модуль Remote Hardware:
   ```shell
   meshtastic --set remote_hardware.enabled true
   ```
