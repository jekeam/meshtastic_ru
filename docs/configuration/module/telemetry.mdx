---
id: telemetry
title: Настройка модуля Telemetry
...
Модуль Telemetry передает по Mesh четыре типа данных: метрики устройства (уровень батареи, напряжение, загрузка канала и airtime) с вашего Meshtastic-устройства, метрики окружения, метрики качества воздуха и метрики здоровья (пульс, сатурация, температура тела).

Поддерживаемые датчики на I2C определяются при старте автоматически. Чтобы собирать показания и отправлять их по Mesh, включите модули Telemetry: Environment, Air Quality и Health.
...
Включить Environment Telemetry (Sensors).
...
Настройки Telemetry доступны в Android.

1. Откройте Meshtastic App
2. Перейдите: **Vertical Ellipsis (3 dots top right) > Radio Configuration > Telemetry**
...
:::note заметка
Все настройки модуля telemetry доступны на iOS, iPadOS и macOS: Settings > Module Configuration > Telemetry (Sensors).
...
Все настройки модуля telemetry доступны в python CLI. Примеры команд ниже:
...
Все настройки модуля telemetry доступны в Web UI.
...
- После следующей загрузки устройство должно показать "Telemetry" и, возможно, данные датчика.
...
И посмотрите Serial-логи для диагностики Telemetry.
