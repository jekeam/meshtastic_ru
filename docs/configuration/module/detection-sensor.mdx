---
id: detection-sensor
title: Использование модуля Detection Sensor
sidebar_label: Detection Sensor
description: Параметры конфигурации модуля Detection Sensor для вашего устройства Meshtastic. Подходит для датчиков движения, герконов и других систем открыто/закрыто.
---
...
Модуль Detection Sensor позволяет настроить GPIO‑пин для мониторинга уровня HIGH/LOW и отправлять текстовые оповещения через portnum Detection Sensor при обнаружении события. Полезно для датчиков движения, герконов и других систем со состоянием открыто/закрыто, где нужны уведомления по Mesh. Параметры: Enabled, Minimum Broadcast Interval, State Broadcast Interval, Send Bell, Name, Monitor Pin, Detection Triggered High и Use Pull-up.

Для работы модуля прошивка устройства должна быть версии 2.2.2 или новее.

## Параметры модуля Detection Sensor {#detection-sensor-module-config-values}
...
## Где доступен модуль Detection Sensor {#detection-sensor-module-client-availability}
...
:::note заметка
Все параметры модуля Detection Sensor доступны на Android в версии приложения 2.2.2 и новее.

1. Откройте приложение Meshtastic
2. Перейдите: **Vertical Ellipsis (3 dots top right) > Radio Configuration > Detection Sensor**
:::
...
:::note заметка
Все параметры модуля Detection Sensor доступны в iOS, iPadOS и macOS, версии 2.2.2 и новее: Settings > Module Configuration > Detection Sensor
:::
...
Все параметры модуля Detection Sensor доступны в python CLI версии 2.2.2 и новее.

:::

Примеры команд ниже:

```shell title="Включить/выключить модуль Detection Sensor"
...
```shell title="Показать конфиг модуля Detection Sensor"
...
Все параметры модуля Detection Sensor доступны в Web UI.
