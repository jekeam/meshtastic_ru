...
2. Go to **⋮ > Конфигурация радио > [Security](/docs/configuration/radio/security/#public-key)** to find its public key.
...
3. Navigate to **Settings > Конфигурация радио > [Security](/docs/configuration/radio/security/#public-key)** to find its public key.
4. Copy the public key to use for configuring the remote node.
5. Connect to the remote node.
6. In **Settings > Конфигурация радио > Security**, add the public key of the local node as an Admin Key.
...
1. Connect to the node that will be used to administer the remote node.
2. Navigate to **⚙ Config > ⚙ Настройки Раидо >[Security](/docs/configuration/radio/security/#public-key)** to find its public key.
3. Copy the public key to use for configuring the remote node.
4. Connect to the remote node.
5. In **⚙ Config > ⚙ Настройки Раидо >Security**, add the public key of the local node as an Admin Key.
