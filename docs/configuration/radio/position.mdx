...
Настройки местоположения отправляются админ-сообщением с protobuf `Config.Position`. Доступные опции: Режим GPS, интервал обновления GPS, фиксированное положение, интеллектуальное вещание, минимальное расстояние интеллектуального вещания, минимальный интервал интеллектуального вещания, интервал вещания, флаги положения и контакты GPS RX/TX/EN.
...
Определяет, какие поля отправляются в сообщениях Местоположение. Значения хранятся как битовое поле булевых опций (побитовое OR PositionFlags).
...
2. Перейдите: **Вертикальное многоточие (3 точки справа вверху) > Конфигурация радио > Местоположение**
...
:::note заметка
Все параметры Местоположение доступны на iOS, iPadOS и macOS: Настройки > Настрйка Устройства > Местоположение.
:::
...
Все команды Местоположение доступны в python CLI. Примеры ниже:
...
Так как устройство делает перезагрузку после каждой команды из CLI, при настройке нескольких значений лучше объединять команды в одну строку. Особенно важно для параметров Местоположение — чтобы они применялись одновременно и не перезаписывались следующими командами.
