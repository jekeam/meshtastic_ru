...
Параметры Network: NTP Server, WiFi Enabled, WiFi SSID, WiFi PSK, Ethernet Enabled, IPv4 Networking Mode, IPv4 Static Address, rsyslog Server и Protocol Flags. Конфиг Network передаётся админ‑сообщением с protobuf `Config.Network`.
...
Параметры Network доступны на Android.

1. Откройте приложение Meshtastic
2. Перейдите: **Вертикальное многоточие (3 dots top right) > Конфигурация радио > Network**
...
Все параметры Network доступны в iOS, iPadOS и macOS: Настройки > Настрйка Устройства > Network.
...
Все параметры Network доступны в Python CLI.
...
:::note заметка
Все параметры Network доступны в Web UI.
:::
